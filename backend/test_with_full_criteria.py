...
async def test():
    ...
    # ✅ ICP with COMPLETE criteria including threshold
    icp = ICP(
        ...
        # ✅ SCORING RULES with threshold
        scoring_rules={
            "criteria_pass_threshold": 60,  # ✅ Need 60+ to pass
            "target_industries": ["E-commerce", "Software", "SaaS", "Technology"],
            "ideal_company_size_min": 100,
            "ideal_company_size_max": 10000,
            "required_technologies": [],  # None required
            "preferred_technologies": ["React", "Python", "Ruby", "Rails"],
    ...
    print("🚀 Testing with FULL ICP criteria (threshold=60)...\n")
    ...
    if 'scoring' in result:
        ...
        print(f"   Passed Criteria: {len(scoring.get('passed_criteria', []))}")
        print(f"   Failed Criteria: {len(scoring.get('failed_criteria', []))}")
    
    if 'enrichment' in result:
        print(f"\n   Enrichment Cost: ${result['enrichment'].get('cost', 0):.4f}")
        print(f"   Fields Added: {len(result['enrichment'].get('fields_added', []))}")
